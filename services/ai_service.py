...
# - model       (gemini-2.5-flash)      -> nur Deck-Analyse (komplexes JSON-Schema).
# - model_lite  (gemini-2.5-flash-lite) -> alles andere (Judge, Übersetzung,
#                                          Roast, Combo-Fallbacks, Vision).
model = None
model_lite = None
...
if KI_VERFUEGBAR and api_key:
    try:
        ...
        model = _GeminiModel(client, "gemini-2.5-flash")
        model_lite = _GeminiModel(client, "gemini-2.5-flash-lite")
        logger.info("Gemini (google-genai SDK) initialisiert: flash + flash-lite.")
    except Exception:
